...
CONCURRENCIA = 3          # Número de descargas simultáneas (recomendado: 3)
PAUSA_SEGURIDAD = 0.5     # Segundos de espera entre bloques
...
def limpiar_nombre_archivo(nombre):
    # Eliminar caracteres prohibidos y acortar si es muy largo
...
async def tarea_capitulo(sem, cap, carpeta_salida, voz, velocidad, titulo_libro, autor, cover_data):
    ...
    async with sem:
        try:
            ...
            print(f"\n❌ Fallo descarga cap {cap['indice']}: {e}")
            return None
...
async def convertir_libro(ruta_epub, velocidad, voz_elegida):
    ...
    print(f"   📑 Capítulos: {len(capitulos)} | 🚀 Hilos: {CONCURRENCIA}")
