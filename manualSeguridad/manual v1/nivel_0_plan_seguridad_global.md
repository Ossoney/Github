...
1. [Introduccion y Objetivos del Manual](#11-introduccion-y-objetivos-del-manual)
2. [Alcance](#12-alcance)
3. [Politica de Seguridad General](#13-politica-de-seguridad-general)
4. [Roles y Responsabilidades (Matriz RACI)](#14-roles-y-responsabilidades-matriz-raci)
5. [Clasificacion de Activos e Informacion](#15-clasificacion-de-activos-e-informacion)
6. [Gestion de Riesgos y Presupuesto](#16-gestion-de-riesgos-y-presupuesto)
7. [Procedimiento Basico de Respuesta a Incidentes](#17-procedimiento-basico-de-respuesta-a-incidentes)
8. [Checklist Nivel 0](#checklist-nivel-0)
...
#### 4. Maquinas Virtuales (VMs) con nginx

Una **Maquina Virtual (VM)** es un entorno de computacion simulado por software que funciona como si fuera una computadora fisica independiente. Permite ejecutar multiples sistemas operativos en un solo equipo fisico.

**nginx** es un servidor web de alto rendimiento que tambien puede funcionar como proxy inverso, balanceador de carga y servidor de correo. Es ampliamente utilizado por su eficiencia y bajo consumo de recursos.

**Ventajas de usar VMs:**
- **Aislamiento:** Los problemas en una VM no afectan a otras ni al sistema anfitrion.
- **Flexibilidad:** Se pueden crear, clonar y eliminar rapidamente.
- **Pruebas seguras:** Ideal para probar configuraciones sin riesgo.

**Ejemplo de instalacion de nginx en Debian:**
```bash
...
|   +------+  +------+  +------+  +------+  +------+       |
|   |  PC  |  |  PC  |  |  PC  |  |  VM   |  |  VM  |      |
|   |Debian|  |Debian|  |Debian|  |nginx  |  |nginx |      |
|   +------+  +------+  +------+  +------+  +------+       |
|                                                          |
+----------------------------------------------------------+
```
...
- Configurar servicios (nginx, SSH, firewall)
...
| Configurar nginx de forma segura | A | **R** | C | - | I |
...
Cuando se publica una vulnerabilidad critica que afecta a nginx:
...
| SW-002 | Software | nginx 1.26 | Servidor Web | Admin. Sistemas | Alta | 2026-02-01 |
| VM-001 | Virtual | VM-nginx-prod | Servidor Web | Admin. Sistemas | Alta | 2026-02-01 |
...
| `/etc/nginx/` | Configuracion del servidor web | Restringida |
...
| R-002 | Vulnerabilidad en nginx | VM nginx | 3 | 4 | 12 | Mitigar: Actualizaciones automaticas |
...
| R-006 | Denegacion de servicio (DoS) | Servidor web | 3 | 3 | 9 | Mitigar: Rate limiting en nginx |
...
| # | Item | Estado | Evidencia/Notas |
|---|------|:------:|-----------------|
| 2.1 | Existe un inventario actualizado de todos los servidores Debian | [ ] | |
| 2.2 | Existe un inventario de equipos de red (routers, switches) | [ ] | |
| 2.3 | Existe un inventario de maquinas virtuales | [ ] | |
...
| # | Item | Estado | Evidencia/Notas |
|---|------|:------:|-----------------|
| 9.1 | nginx esta actualizado a la ultima version estable | [ ] | |
| 9.2 | La version de nginx no se muestra en las respuestas HTTP | [ ] | |
| 9.3 | Las cabeceras de seguridad estan configuradas (X-Frame-Options, etc.) | [ ] | |
| 9.4 | Los archivos de configuracion tienen permisos restrictivos | [ ] | |
| 9.5 | SSL/TLS esta configurado correctamente (si aplica) | [ ] | |
| 9.6 | Los logs de acceso y error estan habilitados | [ ] | |

**Comandos de verificacion:**
```bash
# 9.1 - Version de nginx
nginx -v

# 9.2 - Verificar que no se muestre la version
...
| **VM** | Virtual Machine (Maquina Virtual): entorno de computacion simulado. |
