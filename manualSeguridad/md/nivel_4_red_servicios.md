...
## 5.3. Servicios Web Seguros (nginx/Apache)
...
### 5.3.3. Apache HTTPd - Cabeceras de Seguridad y TLS 1.3

**Apache HTTPd** es el servidor web más popular y ampliamente utilizado. Ofrece capacidades equivalentes a nginx para seguridad, con una configuración y modularidad diferentes.
...
#### Configuración de Cabeceras de Seguridad en Apache

Cree el archivo `/etc/apache2/mods-available/security-headers.conf`:

```apache
# /etc/apache2/mods-available/security-headers.conf
# Cabeceras de seguridad globales para Apache

# HSTS - Forzar HTTPS durante 1 año
Header always set Strict-Transport-Security "max-age=31536000; includeSubDomains; preload"

# CSP - Política de Seguridad de Contenido
Header always set Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.ejemplo.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' https://api.ejemplo.com; frame-ancestors 'self'; form-action 'self'; base-uri 'self'; upgrade-insecure-requests;"
...
Header always set X-Powered-By "Apache"
```
...
sudo ln -s /etc/apache2/mods-available/security-headers.conf /etc/apache2/mods-enabled/

# O incluirlo en la configuración del sitio
# Include /etc/apache2/mods-available/security-headers.conf
```

#### Configuración Completa del Virtual Host con TLS 1.3

Cree el archivo `/etc/apache2/sites-available/ejemplo-seguro-ssl.conf`:

```apache
# /etc/apache2/sites-available/ejemplo-seguro-ssl.conf
# Configuración de Apache con TLS 1.3
...
<VirtualHost *:443>
    ...
    SSLCertificateKeyFile /etc/letsencrypt/live/ejemplo.com/privkey.key
    SSLCertificateChainFile /etc/letsencrypt/live/ejemplo.com/chain.pem
    ...
    # Curvas elípticas permitidas
    SSLECDHCurve X25519:secp384r1

    # Parámetros DH personalizados
...
#### Generar Parámetros DH para Apache

```bash
# Generar parámetros Diffie-Hellman (puede tardar varios minutos)
sudo openssl dhparam -out /etc/apache2/dhparam.pem 4096
sudo chmod 644 /etc/apache2/dhparam.pem
```

#### Habilitar el Sitio y Configuración

```bash
# Crear enlace simbólico del sitio
sudo a2ensite ejemplo-seguro-ssl

# Desabilitar sitio default si es necesario
...
#### Comandos de Verificación Apache

```bash
# Verificar sintaxis de configuración
sudo apache2ctl configtest

# Verificar sintaxis con más detalle
sudo apache2ctl -t -D DUMP_CONFIG
...
# Verificar módulos SSL específicamente
...
# Verificar protocolo y cifrados
...
# Verificar OCSP Stapling
openssl s_client -connect ejemplo.com:443 -status 2>/dev/null | grep -A 5 "OCSP"
...
# Comprobar CSP
curl -sI https://ejemplo.com | grep -i content-security

# Ver versión de Apache
apache2 -v

# Ver configuración cargada del módulo SSL
openssl ciphers -v | head -20

# Ver logs en tiempo real
sudo tail -f /var/log/apache2/ejemplo.com.access.log

# Buscar errores en logs
sudo grep "error\|warn" /var/log/apache2/ejemplo.com.error.log | tail -20

# Probar con herramientas de escaneo
# testssl.sh
./testssl.sh/testssl.sh https://ejemplo.com
```
...
|---------|-------|--------|
| **Cabeceras HSTS** | `add_header` en bloque `server` | `Header always set` en VirtualHost |
| **CSP** | `add_header Content-Security-Policy` | `Header always set Content-Security-Policy` |
| **Ocultación versión** | `server_tokens off;` | `Header always unset Server;` |
| **TLS 1.3** | `ssl_protocols TLSv1.3 TLSv1.2;` | `SSLProtocol +TLSv1.3 +TLSv1.2;` |
| **Cifrados** | `ssl_ciphers ...;` | `SSLCipherSuite ...` |
| **Curvas elípticas** | `ssl_ecdh_curve ...;` | `SSLECDHCurve ...` |
| **OCSP Stapling** | `ssl_stapling on;` | `SSLUseStapling On;` |
| **Verificación sintaxis** | `nginx -t` | `apache2ctl configtest` |
| **Recarga** | `systemctl reload nginx` | `systemctl reload apache2` |
| **Módulos** | Compilados en binario | `a2enmod` / `a2dismod` |
...
### 5.3 Servicios Web (nginx y Apache)

| # | Verificación | Comando | Estado Esperado |
|---|--------------|---------|-----------------|
| 3.1 | nginx activo | `sudo systemctl is-active nginx` | `active` |
| 3.2 | Sintaxis nginx válida | `sudo nginx -t` | `syntax is ok` |
| 3.3 | TLS 1.3 habilitado en nginx | `nginx -T \| grep "ssl_protocols"` | Incluye `TLSv1.3` |
| 3.4 | server_tokens deshabilitado en nginx | `nginx -T \| grep "server_tokens"` | `off` |
...
| 3.8 | OCSP Stapling en nginx | `nginx -T \| grep ssl_stapling` | `on` |
| 3.9 | Apache activo (si instalado) | `sudo systemctl is-active apache2` | `active` |
| 3.10 | Sintaxis Apache válida (si instalado) | `sudo apache2ctl configtest` | `Syntax OK` |
...
*Nivel 4: Red, Servicios y Mantenimiento Avanzado*
*Versión 2.0 - Actualizado con ejemplos de nginx y Apache*
